...
**Default to slide mode (Mode A) unless the user specifically asks for scroll-based.** Slide mode works for both live and async sharing, gives users a familiar navigation experience. Scroll story is a great choice but should be an opt-in, not the default.
...
- **For brand kits, don't overcomplicate it.** You don't need API access or OAuth. Accept hex colors + font names, a logo file — any one of these is enough to apply a brand. Guide the user to export from Canva if they have a Canva template.

---

## Complete Code Examples

### Mode A: Slide Deck (Full Example)

**File**: `output/2026-03-24_skill-intro.html`

See `demo/index.html` for a complete 13-slide example with:
- Fixed 1280x720 canvas with `scaleToViewport()`
- 5 navigation methods (keyboard, click, touch, wheel, dots)
...
### Mode B: Scroll Story (Full Example)

**File**: `output/2026-03-24_annual-report.html`

See `demo/scroll-story.html` for a complete 6-section annual report with:
- CSS scroll-snap mandatory scrolling
- Root font size scaler for responsive layout
...
<!-- Root font size scaler (REQUIRED) -->
<script>
const STORY_REF_WIDTH = 1280;
function scaleStoryLayout() {
  const scale = Math.min(Math.max(window.innerWidth / STORY_REF_WIDTH, 0.5), 2.5);
...
/* All typography in rem — scales with root font size */
h1 { font-size: 5rem; }
h2 { font-size: 3.5rem; }
p { font-size: 1.25rem; }
```
...
### Mode C: Interactive Deck (Full Example)

**File**: `output/2026-03-24_product-demo.html`

See `demo/interactive-deck.html` for a complete 6-slide interactive demo with:
- **Quiz component** with instant feedback
- **Expandable panels** for on-demand details
- **Branch selection** for choose-your-path navigation
- **Data filter** with Chart.js integration
- Standard slide navigation (keyboard, click, dots)
...
<div class="quiz-container">
  <div class="quiz-question">What is the question?</div>
...
document.querySelectorAll('.quiz-option').forEach(option => {
  option.addEventListener('click', () => {
    ...
    const feedback = option.parentElement.parentElement.querySelector('.quiz-feedback');
...
<div class="expandable-section">
  <div class="expandable-item">
    ...
    <div class="expandable-content">
      <div class="expandable-body">Hidden content revealed on click</div>
    </div>
  </div>
</div>
...
<div class="branch-container">
  <div class="branch-card" data-branch="technical">
    <div class="branch-title">Technical Track</div>
    <div class="branch-desc">Deep dive into architecture</div>
  </div>
  <div class="branch-card" data-branch="business">
    <div class="branch-title">Business Track</div>
    <div class="branch-desc">ROI and use cases</div>
  </div>
</div>
...
document.querySelectorAll('.branch-card').forEach(card => {
  card.addEventListener('click', () => {
    ...
    document.getElementById('branch-name').textContent = card.dataset.branch;
...
<script>
const chartData = {
  revenue: { labels: ['Q1', 'Q2', 'Q3', 'Q4'], data: [120, 180, 250, 320], label: 'Revenue ($K)' },
  users: { labels: ['Q1', 'Q2', 'Q3', 'Q4'], data: [5000, 8500, 15000, 22000], label: 'Users' },
  growth: { labels: ['Q1', 'Q2', 'Q3', 'Q4'], data: [15, 50, 76, 47], label: 'Growth (%)' }
};

const ctx = document.getElementById('dataChart').getContext('2d');
let currentFilter = 'revenue';

const chart = new Chart(ctx, {
  type: 'bar',
  data: {
    labels: chartData[currentFilter].labels,
    datasets: [{
      label: chartData[currentFilter].label,
      data: chartData[currentFilter].data,
...
document.querySelectorAll('.filter-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    chart.data.datasets[0].label = chartData[filter].label;
    chart.update();
  });
});
</script>
...
Three complete examples are available in the `output/` directory:

| File | Mode | Description |
|------|------|-------------|
| `2026-03-24_skill-intro.html` | Slide Deck | 13-slide skill introduction with all navigation methods |
| `2026-03-24_annual-report.html` | Scroll Story | 6-section annual report with scroll-triggered animations |
| `2026-03-24_product-demo.html` | Interactive Deck | 6-slide demo with quiz, expandable panels, branching, and data filter |

Open any file in your browser to see the final output quality.
